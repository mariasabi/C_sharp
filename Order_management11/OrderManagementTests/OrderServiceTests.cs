...
namespace OrderManagementTests
{
    public class OrderServiceTests
    {
       ...
       private readonly Mock<IWebHostEnvironment> _webHostEnvironment;
        ...
        [InlineData(7)]
        [InlineData(100)]
        public async void GetItem_InvalidId_ThrowsException(int id)
        {
            var _mockContext = await GetDatabaseContext();
            var _orderRepository = new Order(_mockContext, _webHostEnvironment.Object);

            await Assert.ThrowsAsync<IdNotFoundException>(() => _orderRepository.GetItem(id));
        ...
        [InlineData(7)]
        [InlineData(100)]
        public async void DeleteItem_InvalidId_ThrowsException(int id)
        {
            ...
            await Assert.ThrowsAsync<IdNotFoundException>(() => _orderRepository.DeleteItem(id));
        ...
        [InlineData(3,2)]
        [InlineData(2,20)]
        public async void GetPaginatedItems_InvalidPageNo_ReturnsZeroItems(int pageNo,int pageSize)
        {
            var _mockContext = await GetDatabaseContext();
            var _orderRepository = new Order(_mockContext, _webHostEnvironment.Object); 
            await Assert.ThrowsAsync<ArgumentsException>(() => _orderRepository.GetPaginatedItems(pageNo,pageSize));

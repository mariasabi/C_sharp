...
namespace UserService.Controllers
{
    ...
    public class CartController : ControllerBase
    {
        ...
        public async Task<IActionResult> PurchaseCart(int amt, int cartVal)
        {
            try
            {
                var response = await _cart.PurchaseCart(amt,cartVal);
                return Ok(response);
            }
            catch (ArgumentsException ex)
            {
        ...
        [HttpGet("getVoucherAmount")]
        public async Task<ActionResult<decimal>> GetVoucherAmount()
        {
            try
            {
                var response = await _cart.GetVoucherAmount();
                return Ok(response);
            }
            catch (ArgumentsException ex)
            {
        ...
        [HttpPut("updateVoucherAmount")]
        public async Task<ActionResult<decimal>> UpdateVoucherAmount(decimal amt)
        {
            try
            {
                var response = await _cart.UpdateVoucherAmount(amt);
                return Ok(response);
            }
            catch (ArgumentsException ex)
            {
